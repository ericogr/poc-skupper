...
`README.md` e no `PLAN.md` é montada: topologia de rede, componentes por
cluster, sequência de bootstrap do link, fluxo de tráfego de aplicação,
cadeia de mTLS e defesa em profundidade da unidirecionalidade. Para o
raciocínio por trás de cada decisão (por que Calico só em A, por que
`extraPortMappings` em vez de MetalLB, etc.), ver `PLAN.md`.
